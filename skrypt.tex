...
\end{proof}
\begin{df}
	 Każdemu wyrażeniu języka $\PA$ możemy przypisać kod będący liczbą naturalną, który nazywamy numerem Gödela.
...
\begin{stw}
	 ...
	 \begin{itemize}
		 \item $x$ jest numerem Gödela zmiennej,
		 \item $x$ jest numerem Gödela termu,
		 \item $x$ jest numerem Gödela formuły,
		 \item $x$ jest numerem Gödela koniunkcji formuł o numerach Gödela $y$ i $z$,
		 \item $x$ jest numerem Gödela formuły powstałej z formuły o numerze Gödela $y$ przez dopisanie $\exists$,
		 \item	\ldots,
		 \item $x$ jest numerem Gödela aksjomatu $\PA$,
		 \item $x$ jest numerem Gödela dowodu zdania o numerze $y$ w teorii $T$ ($\text{Prov}_T(x, y)$),
		 \item $\text{num}(x):= \text{ numer Gödela liczebnika }\underline{x}$,
		 \item $\text{subst}(x, y):= \begin{cases}
...
\begin{uw}
	W istocie wszystkie powyższe relacje/wykresy funkcji są definiowalne formułami $\Delta_0(\text{exp})$, tj. formulami klasy $\Delta_0$ z dopuszczeniem termów typu $2^{x^x}$.
\end{uw}
