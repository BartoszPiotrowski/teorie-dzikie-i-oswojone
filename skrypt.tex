...
\begin{ozn}
	 Typy typowo oznaczamy przez $p(x_1, x_2, \ldots, x_n), q(x_1, x_2, \ldots, x_n)$, natomiast:
	 ~\begin{itemize}
		 \item przez $S_n(T)$ oznaczamy zbiór $n$-typów nad $T$,
		 \item przez $S_n^{\A}(D)$ oznaczamy zbiór $n$-typów nad $\A$ z parametrami z $D$.
	 \end{itemize}
\end{ozn}
...
\begin{tw}
	 Jeśli $p$ jest typem nad $\A$ z parametrami z $D$, gdzie $D \subseteq A$,
	 to istnieje $\tilde{\A} \succcurlyeq \A$ realizujący $p$.
\end{tw}
\begin{proof}
	Teoria $\DiagEl(\A) \cup p$ jest niesprzeczna, a jej model jest elementarnym rozszerzeniem $\A$.
\end{proof}
...
\begin{df}[System dowodowy dla FO$(\exists^{\geq \aleph_1})$]
	 Dla dowolnego rozsądnego systemu dowodowego dla FO (w szczegóności umożliwiającego zamianę zmiennych związanych)
	 dodajemy następujące aksomaty i reguły:
	 \begin{enumerate}
